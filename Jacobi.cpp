...
const int totalsize = TOTALSIZE;    // 全局矩阵列数（可变）
...
int main(int argc, char **argv){
    ...
    if (rank == 0) {
        ...
        if (perf_file.is_open()) {
            ...
            perf_file << "文件IO:       " << setw(8) << (io_time/total_time)*100 << " %" << endl;
            perf_file << "其他耗时:     " << setw(8) << (total_time - mem_init_time - comm_total - compute_total - io_time)/total_time*100 << " %" << endl;
            perf_file << "============================================================================================\n" << endl;
            perf_file.close();
        }
    }
